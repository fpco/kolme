...
impl MerkleSerialize for Person0 {
    const MERKLE_SERIALIZE_VERSION: Option<usize> = Some(0);
...
impl MerkleDeserialize for Person0 {
    const MERKLE_DESERIALIZE_VERSION: Option<usize> = Some(0);
...
impl MerkleSerialize for Person1 {
    const MERKLE_SERIALIZE_VERSION: Option<usize> = Some(1);
...
impl MerkleDeserialize for Person1 {
    const MERKLE_DESERIALIZE_VERSION: Option<usize> = Some(1);
    ...
    fn deserialize_previous_version(
        ...
        let name = deserializer.load()?;
        let age = deserializer.load()?;

        let street = if version < 1 {
            DEFAULT_STREET.to_owned()
        } else {
            deserializer.load()?
        };

        Ok(Self { name, age, street })
    }
}
...
impl MerkleSerialize for Person2 {
    const MERKLE_SERIALIZE_VERSION: Option<usize> = Some(2);
...
impl MerkleDeserialize for Person2 {
    const MERKLE_DESERIALIZE_VERSION: Option<usize> = Some(2);

    fn merkle_deserialize(
        deserializer: &mut merkle_map::MerkleDeserializer,
    ) -> Result<Self, MerkleSerialError> {
        // cutting some slack here by reusing code
        Self::deserialize_previous_version(deserializer, Self::MERKLE_DESERIALIZE_VERSION.unwrap())
    }
...
#[tokio::test]
async fn deserializing_previous_version_works() {
    let manager = MerkleManager::default();
    let mut store = MerkleMemoryStore::default();
    ...
    let m0_contents = manager.save(&mut store, &person0).await.unwrap();

    let parsed1: Person1 = manager.load(&mut store, m0_contents.hash).await.unwrap();

    assert_eq!(parsed1.street, DEFAULT_STREET);
}

#[tokio::test]
async fn deserializing_new_version_fails() {
    let manager = MerkleManager::default();
    let mut store = MerkleMemoryStore::default();
    ...
    let person2 = Person2 {
        ...
        age: 31,
        street: String::from("Baker St."),
        zip: 102030,
    };

    let m0_contents = manager.save(&mut store, &person2).await.unwrap();

    let result: Result<Person0, _> = manager.load(&mut store, m0_contents.hash).await;
