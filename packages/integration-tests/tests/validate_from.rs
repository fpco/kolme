...
impl SampleKolmeApp {
    fn new(code_id: u64, validator: PublicKey, approver: PublicKey) -> Self {
...
async fn test_balances_inner(testtasks: TestTasks, (): ()) {
    ...
    let orig_code_id = prepare_local_contract(&submitter_wallet)
        ...
        .unwrap()
        .get_code_id();
    let app = SampleKolmeApp::new(orig_code_id, validator.public_key(), approver.public_key());
    let kolme = Kolme::new(app.clone(), DUMMY_CODE_VERSION, KolmeStore::new_in_memory())
        .await
        .unwrap();
    ...
    testtasks.try_spawn_persistent(Listener::new(kolme.clone(), validator).run(ChainName::Cosmos));

    let discovery = testtasks.launch_websockets_discovery(kolme.clone(), "processor");
    ...
    // Set up the approver using the ValidateFrom sync mode
    let kolme_approver = Kolme::new(app, DUMMY_CODE_VERSION, KolmeStore::new_in_memory())
        .await
        .unwrap();
    testtasks.try_spawn_persistent(Approver::new(kolme_approver.clone(), approver).run());
    testtasks.launch_websockets_client_with(kolme_approver, "Approver", &discovery, |builder| {
        builder.set_sync_mode(
            SyncMode::ValidateFrom {
                state_sync_height: BlockHeight(3),
            },
    ...
    println!("Other secret key: {}", secret.public_key());
