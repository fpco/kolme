...
mod tests {
    ...
    use merkle_map::{MerkleContents, MerkleDeserialize, MerkleDeserializer, MerkleManager, MerkleMemoryStore, MerkleSerialError, MerkleSerialize, MerkleSerializeRaw, MerkleSerializer, Sha256Hash};
    use std::sync::Arc;
    ...
    impl MerkleDeserialize for DummyBytes {
        fn merkle_deserialize(deserializer: &mut MerkleDeserializer, version: usize) -> Result<DummyBytes, MerkleSerialError> {
    ...
    async fn it_serializes_framework_state_app_state_and_logs_as_independent_layers() {
        ...
        // Assert
        assert_eq!(snapshot.len(), 4, "Storable block's contents were not stored in their own layers correctly");
        let stored_framework_state_contents = snapshot.get(&framework_state_merkle.hash).unwrap();
        assert_eq!(stored_framework_state_contents.payload, framework_state_merkle.payload, "Framework state merkle contents were not generated correctly");
        assert!(stored_framework_state_contents.children.is_empty(), "Framework state merkle children were not generated correctly");
        let stored_app_state_contents = snapshot.get(&app_state_merkle.hash).unwrap();
        assert_eq!(stored_app_state_contents.payload, app_state_merkle.payload, "App state merkle contents were not generated correctly");
        assert!(stored_app_state_contents.children.is_empty(), "App state merkle children were not generated correctly");
        let stored_logs_contents = snapshot.get(&logs_merkle.hash).unwrap();
        assert_eq!(stored_logs_contents.payload, logs_merkle.payload, "Logs merkle contents were not generated correctly");
        assert!(stored_logs_contents.children.is_empty(), "Logs merkle children were not generated correctly");
        assert_eq!(storable_block_merkle.children.len(), 3, "Storable block's children were not stored correctly");
        assert_eq!(storable_block_merkle.children.iter().map(|c| c.hash).collect::<Vec<_>>(), vec![framework_state_merkle.hash, app_state_merkle.hash, logs_merkle.hash], "Storable block's children hashes were not stored correctly");
    }
    ...
    async fn it_deserializes_framework_state_app_state_and_logs_from_independent_layers() {
        ...
        let framework_state = Arc::new(DummyBytes(b"Dummy FrameworkState".to_vec()));
        let app_state = Arc::new(DummyBytes(b"Dummy AppState".to_vec()));
        let logs: Arc<[Vec<String>]> = Arc::from(vec![vec!["Dummy".to_owned()], vec!["Logs".to_owned()]]);
        let manager = MerkleManager::new(10);
        let mut store = MerkleMemoryStore::default();
        let block = StorableBlock {
            height: 0,
        ...
        // Act
        let storable_block = manager.deserialize::<StorableBlock<DummyBytes, DummyBytes, DummyBytes>>(storable_block_merkle.hash, storable_block_merkle.payload.clone()).expect("Unable to deserialize storable_block");

        // Assert
        assert_eq!(storable_block.framework_state, framework_state, "Framework state was not deserialized correctly");
        assert_eq!(storable_block.app_state, app_state, "App state was not deserialized correctly");
        assert_eq!(storable_block.logs, vec![vec!["Dummy".to_owned()], vec!["Logs".to_owned()]].into(), "Logs were not deserialized correctly");
    }
}
